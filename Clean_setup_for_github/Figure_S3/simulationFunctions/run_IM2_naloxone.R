# runs the typical 2mg IM naloxone PK simulation
# called by main.R
# inputs:
...
run_IM2_naloxone <- function(fulltimes, pk_parameters, modelFolder){
    # fixed dose of 2 mg
    dose <- 2
    ...
    # update the model parameters with the 2 mg IM naloxone parameters
    truepar <- pars
    ...
    out <- cbind(out, data.frame(group=paste0(dose, "mg IM naloxone")))
