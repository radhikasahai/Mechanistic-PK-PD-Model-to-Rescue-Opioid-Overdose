...
run_IN8_naloxone <- function(fulltimes, pk_parameters, modelFolder){
    ...
    # run the model. Increased maxsteps... some subjects still return early or hit timeout. 
    out <- ode(states, fulltimes, "derivs", truepar, dllname="delaymymod", initfunc="initmod", nout=length(namesyout), rtol=1e-14, atol=1e-14, method="adams", maxsteps=1e5)
    ...
    # prune times not in fulltimes for subjects that return early
    out <- out[out$time %in% fulltimes,] 
