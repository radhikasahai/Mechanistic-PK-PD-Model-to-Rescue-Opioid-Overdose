# runs the typical transmucosal PK simulation
# called by main.R
# inputs:
#   -dose => dose in mg to simulation. Default is 40.7 mg
...
run_TM_fent <- function(dose=40.7, fulltimes, pk_parameters, modelFolder){
    ...
    # update the model parameters with the transmucosal fentanyl parameters
    truepar <- pars
    ...
    out <- cbind(out, data.frame(group=paste0(dose, "mg Transmucosal Fentanyl")))
