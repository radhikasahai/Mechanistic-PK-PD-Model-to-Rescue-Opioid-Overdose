...
double* calc_intermediate(double im11,double im12,double im13,double im14,double im15,double im16,double im18,double im19,double im25,double im26,double im27){
	...
	if (im14<0) /* y[14] = C_B_o2 */
		im14=0;
	// initial brain O2 prssure
	double i_P_B_o2 = im14/alpha_o2;
	// initial venous O2 pressure
...
void initmod(void (* odeparms)(int *, double *)){
	...
	double M_T_o2_real=M_T_o2_0*pow(fracW,P2); // O2 tissue metabolism
...
//	if(Q<Q_0){
	...
	ydot[3] = (1.00/V1C)*((1*kin*y[2])-(kout2*y[3])- (k12N*y[3]))+1/V1C*((k12N*y[28]));
	...
	ydot[27] =0; /* */
	ydot[28] = 1/V2*((k12N*y[3]))-1/V2*((k12N*y[28]));
	...
	yout[7]=P_B_o2;
	yout[8]=P_B_co2;
	yout[9]=y[5]*1e3/VPC; /* opioid plasma */
	yout[10]=y[3]; /* antagonist plasma */
	yout[11]=y[8]; /* opioid effect site */
	yout[12]=y[4]/1e3; /* antagonist effect site  nM*/
